...
supply_risk_factors_int["Supplyrisk"] *= (
    supply_risk_factors_int["Final_Percentage"] / 100
)

# Pre-load metadata
activity_meta = {
    db: {act.key: act for act in Database(db)} for db in CONFIG["databases"]
}

bios_meta = {
    flow.key: {"name": flow["name"], "type": flow["type"]}
...
def get_filtered_activities(db_name):
    db = Database(db_name)
    acts = db.search(CONFIG["filter_keyword"])
...
def compute_lca_flows(db_name, activities):
    ...
    for act in activities:
        ...
        rows_i = [
            ...
            if (amt := lca.supply_array[idx]) > 0 and key in activity_meta[db_name]
        ...
        if not rows_i:
            ...
            df_i = (
                ...
                .merge(
                    supply_risk_factors_int,
                    on=["Activity", "Product", "Geography"],
                    how="inner",
                )
                .assign(
                    risky_int_mass=lambda df: df["Supply_Amount"].where(
                        df["Supplyrisk"] > 0, 0
                    ),
    ...
    for func_str, df_i, df_e in to_plot:
        ...
        def plot_stack(ax, data, value_col, color_col, title):
            ...
            for _, row in d.iterrows():
                h = row[value_col]
                bar = ax.bar(0, h, bottom=bottom, color=row[color_col], width=0.6)[0]
                y_center = bottom + h / 2
        ...
        colmap = {
            ...
            "risky_mass": {
                "intermediate": "risky_int_mass",
                "elementary": "risky_elem_mass",
            },
        ...
        for i, metric in enumerate(metrics):
            for j, (cat, flow, df) in enumerate(panels):
                ...
                grouped = sub.groupby(group_col, as_index=False).agg(
                    {colmap[metric][flow]: "sum", "Color": "first"}
                )
        ...
        plt.subplots_adjust(
            top=0.88, bottom=0.06, left=0.04, right=0.96, hspace=0.4, wspace=0.2
        )
        plt.show()
...
if __name__ == "__main__":
    ...
    for db, cats in all_results.items():
        for cat, df in cats.items():
            ...
            df_mass = df.loc[
                ~((df["risky_int_mass"] == 0) & (df["risky_elem_mass"] == 0))
            ]
