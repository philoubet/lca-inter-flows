...
df_b_public = df_b_public[df_b_public["system process"] == 2][
    ["activityName", "product", "geography"]
]

print(
    f"✅ Extracted {len(df_b_public)} products from B_public.csv (filtered on system process = 2)."
)
...
if os.path.exists(cache_file):
    pubchem_cache = (
        pd.read_csv(cache_file, delimiter=";").set_index("Product")["Formula"].to_dict()
    )
...
manual_matches = {
    row["Product"]: row["Matched Substance"] for _, row in df_matches.iterrows()
}
color_dict = {
    row["Product"]: row["Color"] for _, row in df_matches.iterrows()
}  # Color mapping
...
def molar_mass(formula):
    ...
    try:
        ...
        total_mass = sum(
            periodictable.elements.symbol(element).mass * count
...
element_name_to_symbol = {
    el.name.capitalize(): el.symbol for el in periodictable.elements
}
...
def compute_element_percentage(product, substance):
    ...
    element_mass = (
        periodictable.elements.symbol(substance_symbol).mass
        * parsed_formula[substance_symbol]
    )
...
df_b_public["Atomic_Percentage"] = df_b_public.apply(
    lambda row: (
        compute_element_percentage(row["product"], row["Matched Substance"])
        if pd.notna(row["Matched Substance"])
        else None
    ),
    axis=1,
)
...
df_b_public["Final_Percentage"] = (
    df_b_public["Atomic_Percentage"] * df_b_public["Product_Percentage"]
)
